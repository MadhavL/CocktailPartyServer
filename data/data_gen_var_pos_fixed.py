...
class RoomSimulation:
    ...
    def set_microphones(self, x: float, y: float, z: float, phi: float, mic_offset: np.ndarray):
        ...
        if self.channels == 2:
            ...
            phi += np.pi / 2
        R = pra.beamforming.circular_2D_array([x, y], self.channels, phi0=phi, radius=0.045)
...
class SPRoomSimulator:
    ...
    def create_sample(self, 
                        ...
                        seed2: int, 
                        target_angle: float = 0,  # This will be ignored as we fix positions
                        reverb: bool = True, 
                        rt60_min:float=0.2,
                        rt60_max:float=1, 
                        snr_min: int =-10, 
                        ...
                        min_angle_dist: int = 10, ):  # This will be ignored as we fix positions
        """
        Create for a list of speech signals (first one is the target signal) the spatial image using a randomly placed
        microphone array and fixed speaker positions:
        - Target speaker at 0 degrees
        - Interferer 1 at 90 degrees
        - Interferer 2 at 120 degrees

        All other parameters remain the same as in the original implementation.
        """
        ...
        # Fixed angles for speakers (in degrees)
        speaker_angles = [0, 90, 120]  # target, interferer1, interferer2
        speaker_phis = []
        
        # Place all speakers
        for idx, (angle, signal_data) in enumerate(zip(speaker_angles, signal)):
            speaker_phi = phi + angle / 360 * 2 * np.pi
            speaker_phis.append(speaker_phi)
            
            source_pos = mic_pos + normal_vec(speaker_phi) * (self.rng.random() * (max_dist-min_dist) + min_dist)
            source_pos[2] = self.rng.normal(1.60, 0.08)  # height of speaker
            
            if idx == 0:
                # Target speaker
                self.exp_room.add_source(source_pos, signal_data, 0)
                self.dry_room.add_source(source_pos, signal_data, 0)
                meta["target_file"] = speaker_list[0].split("wsj0")[-1].replace("\\", "/")
                meta["target_pos"] = source_pos.tolist()
                meta["target_angle"] = 0  # Fixed at 0 degrees
            else:
                # Interfering speakers
                self.exp_room.add_source(source_pos, signal_data, 0)
                meta[f"interf{idx-1}_file"] = speaker_list[idx].split("wsj0")[-1].replace("\\", "/")
                meta[f"interf{idx-1}_pos"] = source_pos.tolist()
